        ...
        for split, wsi_sub in {"train":train_wsi, "valid":valid_wsi, "test":[test_wsi]}.items():
            ...
            for c_idx, c in enumerate(tqdm(selected_cluster,
                ...
                if len(idxs)>MAX_PATCHES:
                    logging.warning(f"簇 {c} patch={len(idxs)} 超过 {MAX_PATCHES}, 跳过")
                    continue
                ...
                logging.info(f"簇 {c} 推理完成, patch={len(idxs)}")

            logging.info(f"[{split}] 推理耗时 {time.time()-t0:.1f}s")

            # ---- 新增：如果 sum_feat 仍然 None，说明所有簇都被跳过 ----
            if sum_feat is None:
                logging.warning(f"[{split}] 没有可用簇，跳过CSV保存")
                continue

            # 拉回 CPU，做平均并保存
            sf = sum_feat.cpu().numpy()
            sr = sum_risk.cpu().numpy()
